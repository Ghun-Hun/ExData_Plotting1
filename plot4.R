...
with(house, {
  #first plot
    plot(date,Global_active_power,ylab="Global Active Power",xlab=" ",type="l",axes=FALSE,frame.plot=TRUE)  
  ...
  #third plot
    plot(date,Voltage,ylab="Voltage",xlab="datetime",type="l",axes=FALSE,frame.plot=TRUE)  
    axis.POSIXct(1,seq(min(date),max(date),"days"),format="%a")
    axis(2,at=seq(234,246,2))
  #fourth plot
   plot(date,Global_reactive_power,ylab="Global_reactive_power",xlab="datetime",type="l",axes=FALSE,frame.plot=TRUE)  
   axis.POSIXct(1,seq(min(date),max(date),"days"),format="%a")
